package net.creeperhost.minetogetherservers.server;
...
import net.creeperhost.minetogetherservers.server.web.ServerListUpdateRequest;
...
public class ServerListThread extends Thread {
    ...
    private String secret = MineTogetherServers.FINGERPRINT;
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ApiClientResponse<ServerListUpdateRequest.Response> resp = MineTogetherServers.API.execute(new ServerListUpdateRequest(
                        serverIp,
                        secret,
                        displayName,
                        projectId,
                ...
                if (!response.getStatus().equals("error")) {
                    tries = 0;
                    MineTogetherServersServer.inviteId = response.id;
