...
public class Log4jUtils {
    ...
    public static void attachMTLogs(Path logsFolder) {
        ...
        try {
            ...
            configuration.getRootLogger().addAppender(latest, Level.INFO, new LoggerNameStartsWithFilter("net.creeperhost.minetogetherservers", Result.ACCEPT, Result.DENY));
            configuration.getRootLogger().addAppender(debug, Level.ALL, new LoggerNameStartsWithFilter("net.creeperhost.minetogetherservers", Result.ACCEPT, Result.DENY));
            ctx.updateLoggers();
        } catch (Throwable ex) {
            LOGGER.error("Unable to configure MineTogether Servers Logging.", ex);
        }
    }
}
